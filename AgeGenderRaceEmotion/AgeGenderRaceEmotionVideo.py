...
# Load YouTube video
url = "https://www.youtube.com/watch?v=eV74Wtmv0H4"
video = pafy.new(url)
...
try:
    while cap.isOpened():
        ...
        for face in yuz:
            ...
            emotion_confidence = np.max(results) * 100

            # Print the detected attributes
            print(f"Race: {race} ({race_confidence:.2f}%)")
            print(f"Age: {age} ({age_confidence:.2f}%)")
            print(f"Gender: {gender} ({gender_confidence:.2f}%)")
            print(f"Emotion: {emotion_label} ({emotion_confidence:.2f}%)")
            print()

            # Display face attributes on the image
            cv2.putText(frame, f"{race}/{age}/{gender}/{emotion_label}", (x, y - 8), font, 0.8, (0, 255, 255), 2,
                        cv2.LINE_AA)

        # Display the image with detected face attributes
